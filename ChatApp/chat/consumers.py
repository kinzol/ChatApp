...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data=None, bytes_data=None):
        if bytes_data:
            ...
            crypt_rand_string = ''.join(secrets.choice(letters_and_digits) for i in range(10))
            ...
            await database_sync_to_async(self.save_audio)(content=media_path,chat_id=self.scope["url_route"]["kwargs"]["room_name"], )
            ...
            await self.channel_layer.group_send(
                ...
                {"type": "chat.message", 'message_type': 'audio', "message": media_path, 'username': self.scope['user'].username, }
            )
        ...
        if text_data:
            ...
            await self.channel_layer.group_send(
                self.room_group_name, {"type": "chat.message", 'message_type': 'text', "message": message, 'username': username,}
            )
    ...
    def save_audio(self, content, chat_id):
        new_message = Messages.objects.create(message_type='audio', user=self.scope["user"], content=content, chat_id=chat_id)
    ...
    async def chat_message(self, event):
        ...
        await self.send(text_data=json.dumps({'message_type': message_type, "message": message, 'username': username, 'time': timezone.now().strftime('%H:%M')}))
...
class GroupConsumer(AsyncWebsocketConsumer):
    ...
    def save_audio(self, content, chat_id):
        new_message = GroupMessages.objects.create(message_type='audio', user=self.scope["user"], content=content, chat_id=chat_id)
        return new_message.pk
